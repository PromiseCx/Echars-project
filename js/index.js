...
;(function() {
 ...
 $(".filter").on("click","a",function() {
  ...
  $(".inner>.data").eq(index).show().siblings(".data").hide();
});
...
(function() {
  ...
  var option = {
    tooltip: {
      // 触发方式，item表示将数据放到对应的表格上去触发
      trigger: "item",
      /*
        格式化提示内容，
        a:表示series里面的系列名称
        b：表示series里面的data里面的name，
        c:表示series里面的data里的value，
        d：表示series里面的 当前数据 / 总数据 的比例
      */  
...
(function() {
  ...
  var option = {
    ...
    // y轴上面的数值是根据其series里面的data数值自动计算的
    series: [
      {
        name: "直接访问",
        type: "bar",
    ...
    series: [
      {
        ...
        // 饼形图的起始角度为 180， 注意不是旋转角度,默认值为90
        startAngle: 180,
        // 鼠标经过不需要放大偏移图形，默认值为10
        hoverOffset: 0,
        data: [
          {
            value: 100,
...
/*
    注意hover和mouseenter事件，可能会发生冲突，要注意
*/
